...
def test_submodules(utils, tmpdir):
    ...
    # Check the content of repository (Python module "filabel")
    content = os.listdir(str(tmpdir.join(repo_dir)))
    assert 'filabel' in content, \
        'No "filabel" entry in the repository'

    assert os.path.isdir(str(tmpdir.join(repo_dir).join('filabel'))), \
        'Entry "filabel" is not a directory'

    content = os.listdir(str(tmpdir.join(repo_dir).join('filabel')))

    assert '__init__.py' in content, \
        'Filabel is not a module (no __init__.py file)'
    assert '__main__.py' in content, \
        'Filabel is not runnable (no __main__.py file)'
