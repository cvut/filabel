...
mimo původní aplikace půjde tedy nevíc spustit web server a to pomocí `flask run`.
...
Nebudeme hodnotit vzhled stánky,
...
V konfiguračních souborech hledáte sekce `github` a `lables`.
...
tak tento request přišel z GitHubu, a můžete mu tydy veřit.
...
Žádné nové automatciké testy nyní nejsou.
Ujistěte se, že vaše nasazená aplikace funguje, jak má,
a že testy z minula stále prochází.

K minulému zadání existuje sada testů.
